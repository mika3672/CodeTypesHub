(function (data_attribute_stringified) {
    ...
    if (elements1.length > 0) {
        ...
        imageUrls = Array.from(elements2).map((el) => el.getAttribute(`data-original-src`));
    }

    let picLinks = window.prodVariantsColor.map((el) => el?.featured_image?.src).filter((e) => e);

    if (!imageUrls.length) imageUrls = picLinks.length ? picLinks : window.productInfo.pictures;
